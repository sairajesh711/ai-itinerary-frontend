	...
	const STAGES = [
		{
			...
			title: 'Consulting the Atlas…',
			sub: 'Laying out the maps for your destination.'
		},
		{
			start: 0.15,
			end: 0.35,
			title: 'Finding the Hidden Gems…',
			sub: 'Scanning for local festivals and secret viewpoints.'
		},
		{
			start: 0.35,
			end: 0.6,
			title: 'Plotting your Journey…',
			sub: 'Sketching the most delightful routes between sights.'
		},
		{
			start: 0.6,
			end: 0.85,
			title: 'Chatting with the Locals…',
			sub: 'Finding the best cafés and coziest stays.'
		},
		{
			start: 0.85,
			end: 0.95,
			title: 'Balancing the Budget…',
			sub: 'Making sure the numbers add up perfectly.'
		},
		{
			start: 0.95,
			end: 1.0,
			title: 'Adding the Finishing Touches…',
			sub: 'Your adventure is almost ready!'
		}
	];
			...
			<div class="canvas">
				<svg viewBox="0 0 900 520" class="sheet" aria-hidden="true">
					...
					<path
						...
						class="signature {current(5) ? 'signature-anim' : ''} {active(5)
							? ''
							: 'signature-hidden'}"
					/>
				</svg>
...
<style>
	...
	.route-anim {
		animation: dash 2200ms cubic-bezier(0.2, 0.7, 0.1, 1) forwards;
	}
	@keyframes dash {
		from {
	...
	.signature-anim {
		...
		animation: write 900ms ease-out forwards;
	}
	@keyframes write {
		to {
			stroke-dashoffset: 0;
	...
	@keyframes shimmer {
		...
		50% {
			box-shadow: inset 0 0 60px rgba(0, 0, 0, 0.035);
		}
	}
</style>
