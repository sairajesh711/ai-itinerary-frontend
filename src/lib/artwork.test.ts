...
describe('artwork resolution', () => {
	it('returns default artwork for empty/null/undefined input', () => {
		...
		expect(resolveArtwork(undefined)).toEqual({
			src: '/art/default.webp',
			opacity: 0.14,
			nudgeVw: 0
		});
		expect(resolveArtwork('   ')).toEqual({ src: '/art/default.webp', opacity: 0.14, nudgeVw: 0 });
	...
	it('resolves major cities correctly', () => {
		...
		expect(resolveArtwork('london')).toEqual({
			src: '/art/london.webp',
			opacity: 0.14,
			nudgeVw: -2
		});
	...
	it('resolves countries correctly', () => {
		...
		expect(resolveArtwork('Brazil')).toEqual({
			src: '/art/brazil.webp',
			opacity: 0.14,
			nudgeVw: -6
		});
	...
	it('resolves regions correctly', () => {
		expect(resolveArtwork('Southeast Asia')).toEqual({
			src: '/art/southeast.webp',
			opacity: 0.14,
			nudgeVw: -3
		});
		expect(resolveArtwork('Middle East')).toEqual({
			src: '/art/middle-east.webp',
			opacity: 0.14,
			nudgeVw: -4
		});
		expect(resolveArtwork('Central Asia')).toEqual({
			src: '/art/central-asia.webp',
			opacity: 0.14,
			nudgeVw: -3
		});
	});
	...
	it('handles partial matches correctly', () => {
		expect(resolveArtwork('I want to visit Paris next month')).toEqual({
			src: '/art/paris.webp',
			opacity: 0.14,
			nudgeVw: 0
		});
		expect(resolveArtwork('Trip to Japan and Korea')).toEqual({
			src: '/art/japan.webp',
			opacity: 0.14,
			nudgeVw: -2
		});
	...
	it('falls back to regional defaults for generic terms', () => {
		expect(resolveArtwork('somewhere in Europe')).toEqual({
			src: '/art/eastern-europe.webp',
			opacity: 0.14,
			nudgeVw: -3
		});
		...
		expect(resolveArtwork('Middle East region')).toEqual({
			src: '/art/middle-east.webp',
			opacity: 0.14,
			nudgeVw: -4
		});
	...
	it('returns default for unrecognized locations', () => {
		...
		expect(resolveArtwork('Atlantis')).toEqual({
			src: '/art/default.webp',
			opacity: 0.14,
			nudgeVw: 0
		});
		expect(resolveArtwork('Random City Name')).toEqual({
			src: '/art/default.webp',
			opacity: 0.14,
			nudgeVw: 0
		});
	...
	it('handles special location aliases', () => {
		expect(resolveArtwork('Taj Mahal')).toEqual({
			src: '/art/taj.webp',
			opacity: 0.14,
			nudgeVw: -5
		});
		expect(resolveArtwork('visiting the taj')).toEqual({
			src: '/art/taj.webp',
			opacity: 0.14,
			nudgeVw: -5
		});
		expect(resolveArtwork('New York City')).toEqual({
			src: '/art/nyc.webp',
			opacity: 0.14,
			nudgeVw: -2
		});
