...
{#if leg}
	<div
		...
		role="img"
		aria-label="Travel from previous activity: {leg.mode} for {leg.duration_minutes ||
			'unknown'} minutes"
	>
		<!-- Animated Path -->
		...
		{#if chipVisible}
			<div class="travel-chip" in:fade={{ duration: ANIMATIONS.DURATIONS.NORMAL, delay: 100 }}>
