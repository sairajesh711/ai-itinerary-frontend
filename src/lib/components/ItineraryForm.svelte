<script lang="ts">
	...
	const INTERESTS: { key: string; label: string; svg: string }[] = [
		...
		{ key: 'museum', label: 'Museum', svg: '<path d="M3 6l5-3 5 3v5H3z" stroke-width="1.5"/>' },
		{
			key: 'nightlife',
			label: 'Nightlife',
			svg: '<path d="M4 11l6-6 2 2-6 6z" stroke-width="1.5"/>'
		},
		{ key: 'nature', label: 'Nature', svg: '<path d="M8 3v8M5 7h6" stroke-width="1.5"/>' },
		{
			key: 'shopping',
			label: 'Shopping',
			svg: '<path d="M4 6h8l-1 6H5z M6 6V5a2 2 0 014 0v1" stroke-width="1.5"/>'
		},
		{
			key: 'beach',
			label: 'Beach',
			svg: '<path d="M3 11h10M5 9c2-3 5-3 7 0" stroke-width="1.5"/>'
		},
		{ key: 'hike', label: 'Hike', svg: '<path d="M5 11l3-6 3 6" stroke-width="1.5"/>' },
		{
			key: 'coffee',
			label: 'Coffee',
			svg: '<path d="M4 9h7a2 2 0 100-4H4v4z" stroke-width="1.5"/>'
		},
	...
	const TRANSPORT: { key: Transport; label: string; svg: string }[] = [
		{
			key: 'walk',
			label: 'Walk',
			svg: '<path d="M6 3a1 1 0 102 0 1 1 0 10-2 0z M6 5v3l-2 3m4-6l2 2 1 4" stroke-width="1.5"/>'
		},
		{
			key: 'public_transit',
			label: 'Transit',
			svg: '<path d="M4 4h8v6H4zM5 11h2m4 0h2" stroke-width="1.5"/>'
		},
		{
			key: 'train',
			label: 'Train',
			svg: '<path d="M4 4h8v6H4zM5 10l-1 2m7-2l1 2" stroke-width="1.5"/>'
		},
		{ key: 'car', label: 'Car', svg: '<path d="M4 8h8l-1-3H5zM5 10h2m4 0h2" stroke-width="1.5"/>' },
		{
			key: 'bike',
			label: 'Bike',
			svg: '<path d="M5 10a2 2 0 100-4 2 2 0 000 4zm6 0a2 2 0 100-4 2 2 0 000 4z M6 8l3-3" stroke-width="1.5"/>'
		},
	...
	function submit() {
		...
		const sanitizedDestination = sanitizeUserInput(destination, 100);
		if (!sanitizedDestination.trim()) {
		...
		const validTravelers = Math.max(1, Math.min(12, Number(travelers_count) || 1));
		const validDuration = Math.max(1, Math.min(30, Number(duration_days) || 3));
		const validBudget = max_daily_budget
			? Math.max(0, Math.floor(Number(max_daily_budget)))
			: undefined;

		const base: Partial<ItineraryRequest> = {
			destination: sanitizedDestination,
			start_date,
			interests: interests.map((interest) => sanitizeUserInput(interest, 50)).filter(Boolean),
			travelers_count: validTravelers,
			budget_level,
			pace,
			language: 'en',
			preferred_transport
		...
		if (validBudget != null && !Number.isNaN(validBudget)) {
			base.max_daily_budget = validBudget;
		}

		if (home_currency) {
			base.home_currency = sanitizeUserInput(home_currency, 5);
		}

		if (end_date) {
			base.end_date = end_date;
		} else {
			base.duration_days = validDuration;
		}
	...
	<div class="mt-6">
		...
		<div class="mt-3 grid grid-cols-2 sm:grid-cols-3 gap-2">
			{#each INTERESTS as it}
				...
				>
					<!-- SVG paths are hardcoded and safe from XSS -->
		...
		<div>
			...
			<div class="seg mt-2" role="group" aria-label="Budget level">
				{#each ['shoestring', 'moderate', 'comfortable', 'luxury'] as any as b}
					<button
						type="button"
						class="seg-btn {budget_level === b ? 'seg-active' : ''}"
		...
		<div>
			...
			<div class="seg mt-2" role="group" aria-label="Trip pace">
				{#each ['relaxed', 'balanced', 'packed'] as any as p}
					<button
						type="button"
						class="seg-btn {pace === p ? 'seg-active' : ''}"
		...
		<div class="mt-4 grid grid-cols-1 sm:grid-cols-3 gap-4">
			...
			<div class="sm:col-span-2">
				...
				<div
					...
					role="group"
					aria-label="Preferred transport methods"
				>
					{#each TRANSPORT as t}
						<button
							type="button"
