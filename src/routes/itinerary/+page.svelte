<script lang="ts">
	...
	function scrollToSelectedDay() {
		const dayComponent = dayComponents.find(
			(_, i) => currentItinerary?.daily_plan[i]?.day_index === $ui.selectedDay
		);
			...
			<div class="container-pro py-4">
				<div class="flex items-center justify-between">
					...
					<div class="text-center">
						<h1 class="font-heading text-2xl sm:text-3xl font-semibold tracking-tight">
							{currentItinerary.destination}
						</h1>
						<p class="text-sm text-slate-600 mt-1">
							{currentItinerary.total_days} day{currentItinerary.total_days !== 1 ? 's' : ''} •
							{currentItinerary.travelers_count || 1} traveler{(currentItinerary.travelers_count ||
								1) !== 1
								? 's'
								: ''}
						</p>
