<script lang="ts">
	...
	export let stageFractions = [0.12, 0.18, 0.25, 0.2, 0.15, 0.1];
	...
	function computeBaseProgress(elapsed: number) {
		...
		const nextAnchor =
			i + 1 < stageAnchors.length ? stageAnchors[i + 1] : stageAnchors[stageAnchors.length - 1];

		// While pending, cap the final anchor so we never look “stuck at 100%”
		const targetAnchor =
			i === stageAnchors.length - 1 && backendState !== 'done'
...
{#if visible}
	<div class="fixed inset-0 z-50 bg-black/25 flex items-center justify-center p-4">
		<div
			class="w-full max-w-xl rounded-2xl border border-slate-200 bg-[var(--paper,#f7f5f1)] shadow-xl p-6"
		>
